...
const EditContactForm: FC<EditContactFormProps> = ({
  ...
  const { address, isLoading, isError, errorMessage } = useCEP(
    shouldFetch ? rawCep : ""
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      const cpfExists =
        contact.cpf !== form.cpf &&
        getContactByCpf(user?.email || "anon", form.cpf);
      if (cpfExists) {
        setErrors((prev) => ({
          ...prev,
      ...
      {isLoaded && coords?.lat && coords?.lng ? (
        ...
        <div className={styles.illustrationContainer}>
          <h3 className={styles.title}>
            Preencha os dados e a prévia do seu mapa aparecerá aqui
          </h3>
          <img
            className={styles.illustration}
            src={MapImageGirl}
            alt="Ilustração de mapa"
          />
        </div>
      )}
